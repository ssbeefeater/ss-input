(function(e,a){function b(b){var c;e(window).bind("beforeunload",function(){b.$element.trigger("resetAction.ssi").trigger("closeAction.ssi").off(".ssi")});e(window).on("resize."+b.uniqueId,Ss_input.tools.debounce(function(){d("",b)},200));b.$content.on({"mouseenter.ssi":function(b){b.preventDefault();c=e(this).find(".ssi-optionsDiv");c.fadeIn(200)},"mouseleave.ssi":function(){!1===c.hasClass("selected")&&c.fadeOut(200)}},".ssi-displayDiv");b.$element.on("closeAction.ssi",function(){a.removeAll();
...
a.options.showTo?115:70));a.$content.hasClass("ssi-multiPickMode")&&(b+=40);var c=parseInt(a.$content.parent().height())-b;a.$content.find("#ssi-mainContent").css("height",c)}var f={},g=0;this.Ss_input=function(b,a){this.ssi_excludeList={};this.options=e.extend(!0,{plugins:[],modalOptions:{sizeClass:"large",fixedHeight:!0,fitScreen:!0,buttons:[]},permissions:["all"],excludePlugin:[],language:"en",mode:"selection",storageType:"fileSystem",showTo:"modalWindow",responseValidation:!1},a);this.requireAfter=
["scan"];this.uniqueId=g;this.readOnlyMode=!1;this.plugins=[];this.pluginNames=[];this.handlersGroup="handlers";this.corePlugins=[];this.initializedButtons={};this.readyState=!1;this.language=Ss_input.locale[this.options.language];this.$element=e(b);g++;this.init()};Ss_input.prototype.buttonSelectors={menu:"#ssi-menuButtons",contextMenu:"#ssi-contextMenuUl",options:"#ssi-TasksBtn+ul",actions:"#ssi-actionsBtn+ul",items:".ssi-optionsDiv",bottom:"#ssi-rightButtons",openWith:"#ssi-openWithBtn ul",editWith:"#ssi-editWithBtn ul"};
Ss_input.prototype.init=function(){var b=Ss_input.tools.keyExists(this.options.storageType,Ss_input.storageTypes),b=b.concat("manager"!=this.options.mode?Ss_input.tools.keyExists(this.options.mode,Ss_input.modePlugins):[],Ss_input.tools.keyExists(this.require,Ss_input.plugins));if(0!=b.length)console.log("Some requirements are missing: "+b.toString());else{if(0<this.options.plugins.length){if(this.pluginNames=this.options.plugins,b=Ss_input.tools.arrayValuesInArray(this.require,this.pluginNames))this.pluginNames=
this.pluginNames.concat(b)}else for(var a in Ss_input.plugins)this.pluginNames.push(a);this.pluginInit(this.options.storageType,Ss_input.storageTypes);this[this.options.storageType]=this.corePlugins[this.options.storageType];"manager"!=this.options.mode?(this.pluginInit(this.options.mode,Ss_input.modePlugins),this[this.options.mode]=this.corePlugins[this.options.mode]):("modalWindow"!=this.options.showTo&&(this.options.showTo=this.$element),this.$element.addClass("ss-input"));if("modalWindow"===this.options.showTo)h(this);
else if(b=e(this.options.showTo),b.is("div")){if(this.$element.trigger("earlyShowAction"),a=c(this))b.first().addClass("ssi-mainModal").html(a),d(null,this)}else console.log("Use a div.")}};var h=function(b){e.extend(b.options.modalOptions,{className:"ssi-mainModal",onClose:function(){b.$element.trigger("resetAction.ssi").trigger("closeAction.ssi")}});b.$element.on("click",function(a){a.preventDefault();b.$element.trigger("earlyShowAction");if(e(a.target).hasClass("ss-input")){a=b.createWindow(b.options.modalOptions);
var f=c(b);f?(a.setContent(f),d(null,b)):a.close()}})};Ss_input.prototype.destroy=function(){this.$element.trigger("closeAction.ssi");this.$content.remove()};Ss_input.prototype.checkPermissions=function(b,a){return("read"===b||"all"===this.options.permissions[0]||0===Ss_input.tools.arrayValuesInArray(b,this.options.permissions).length||-1!==e.inArray(a,this.options.permissions))&&(-1===e.inArray(a,this.options.excludePlugin)||-1!==e.inArray(a,this.require)||-1!==e.inArray(a,this.requireAfter))};Ss_input.prototype.pluginInit=
function(b,a){b=Ss_input.tools.toArray(b);for(var c=0,d=b.length;c<d;c++)try{a=a||Ss_input.plugins;if(0!=Ss_input.tools.keyExists(b[c],a).length)throw"did not load";if(b[c]){var f=new a[b[c]](this,this.options[b[c]]);if(this.checkPermissions(f.permissions,b[c])&&!e.isEmptyObject(f)&&0!==Ss_input.tools.keyExists(b[c],this[f.type]).length){var g=Ss_input.tools.keyExists(f.require,a);if(0===g.length){var k=Ss_input.tools.keyExists(f.require,this[f.type]);this.pluginInit(k);if(0===Ss_input.tools.keyExists(k,
this.plugins).length||0===Ss_input.tools.keyExists(k,this.corePlugins).length){this[f.type][b[c]]=f;if(Ss_input[this.handlersGroup].hasOwnProperty(b[c])){var h=b[c];new Ss_input[this.handlersGroup][h](this,f,h)}else if(Ss_input.handlers.hasOwnProperty(b[c])){var l=b[c];new Ss_input.handlers[l](this,f,l)}f.init()}else throw"requires "+k.toString()+".";}else throw"requires "+g.toString()+".";}}}catch(v){console.log(b[c]+" "+v)}};Ss_input.prototype.resetPlugins=function(b){for(var a in b){var c=b[a];
Ss_input[this.handlersGroup].hasOwnProperty(a)?new Ss_input[this.handlersGroup][a](this,c,a):Ss_input.handlers.hasOwnProperty(a)&&new Ss_input.handlers[a](this,c,a);c.init()}};Ss_input.prototype.addButton=function(b,a,c,d){var g={menuButton:["menu","bottom"],listButton:["options","actions","contextMenu","openWith","editWith"],itemButton:[]};a=a||"menuButton";if("bottom"==c&&"modalWindow"===this.options.showTo)this.options.modalOptions.buttons.push(b);else{var k=new Ss_input.Button(b,a,this.$content);
if(c)if(c instanceof e)c.append(k);else if("itemButton"!=a){for(var h=[],l=0,m=c.length;l<m;l++)-1!==e.inArray(c[l],g[a])&&h.push(this.buttonSelectors[c[l]]);f.hasOwnProperty(h.toString())?f[h.toString()].push(k):f[h.toString()]=[k];d&&b.selectionRequired&&(this.ssi_excludeList[b.className]=d)}return k}};Ss_input.prototype.notify=function(b,c){a.notify(b,{overrideOther:!0,content:c})};Ss_input.prototype.checkExcludedButtons=function(b){var a=this.ssi_excludeList,c=this.get$selectedItems(),d=this.$content.find(".ssi-mustSelect").removeClass("ssi-hidden disabled"),
f;for(f in a)c.each(function(){e(this).data("info");if("false"==eval(Ss_input.tools.dataReplace(a[f],"info"))){var c=d.filter("."+f).addClass("ssi-hidden");"function"===typeof b&&b(c);return!1}})};Ss_input.prototype.createWindow=function(b,c){return a.show(b,c)};Ss_input.prototype.get$mainElementById=function(b,a){a=a||this.$content;return a.find(".ssi-mainElement[data-ID='"+b+"']")};Ss_input.prototype.get$mainElement=function(b){return b.hasClass("ssi-mainElement")?b:b.parents(".ssi-itemWrapper").find(".ssi-mainElement")};
Ss_input.prototype.getId=function(b){if(b)return"string"===typeof b?this.get$mainElementById(b).attr("data-ID"):this.get$mainElement(b).attr("data-ID");var a=[];this.get$selectedItems().each(function(){a.push(e(this).attr("data-ID"))});return a};Ss_input.prototype.get$itemWrapper=function(b){return b.parents(".ssi-itemWrapper")};Ss_input.prototype.get$selectedItems=function(b){try{var a=this.plugins.select;return b?this.get$mainElement(b):0<a.selectionList.length?e(".ssi-mActive"):this.plugins.contextMenu.rightClickItem||
...
this,a=this.ssi;a.$element.on("showAction",function(){b.options.duplicate||(b.checkItem(b.selected),b.checkedFilesCount=b.selected.length);a.$content.addClass("ssi-multiPickMode");a.$content.find("#ssi-menuButtons").after(Ss_input.tools.template(b.template.displayFiles,{selected:b.options.maxItems?'<span id="ssi-displaySelectedFiles">'+b.selectedFilesCount+"</span>/"+b.options.maxItems:'<span id="ssi-displaySelectedFiles">'+b.selectedFilesCount+"</span>",checked:b.checkedFilesCount,selectedTooltip:b.translate("selectedTooltip"),
checkedTooltip:b.translate("checkedTooltip"),checkedBtn:b.translate("checkedBtn"),selectedBtn:b.translate("selectedBtn")}));a.$content.on("click","#ssi-clearChecked",function(){b.clearChecked();return!1}).on("click","#ssi-clearSelected",function(){b.clearSelected();return!1});a.$element.on("echoItemsAction.ssi silentEchoItemsAction.ssi",function(){b.setMChecked()}).on("closeAction.ssi",function(){b.checkedFilesCount=0;b.checkedItems=[]}).on("appendItemAction.ssi",function(a,c){-1!==e.inArray(c.data.path,
b.checkedItems)&&b.addCheck("",c.$element.find(".ssi-canSelect"))}).on("infoCloseAction.ssi",function(){b.setMChecked()}).on("infoShowAction.ssi",function(a,c){b.setMChecked(c)}).on("beforeChangeCollectionAction.ssi",function(){b.selectItems();b.reset()}).on("removeItemAction.ssi",function(a,c){var d=e.inArray(c,b.checkedItems);console.log(d);console.log(b.checkedItems)})});e(this.options.content).eq(0).on("click",".ssi-removeChoice",function(a){try{b.removeSelectedImage(a),b.selectedFilesCount--,
b.ssi.$content.find("#ssi-displaySelectedFiles").html(b.selectedFilesCount)}catch(f){}return!1}).on({mouseenter:function(){e(this).find("#ssi-actions").fadeIn(200)},mouseleave:function(){e(this).find("#ssi-actions").fadeOut(200)}},".ssi-selectionPreview")},reset:function(){this.checkedFilesCount=0;this.checkedItems=[];this.options.duplicate?(this.ssi.$content.find("#ssi-displayCheckedFiles").html("0"),this.ssi.$content.find(".ssi-checked").removeClass("ssi-checked").find(".ssi-checkItem").remove()):
(this.checkItem(this.selected),this.checkedFilesCount=this.selected.length);this.ssi.$content.find("#ssi-displaySelectedFiles").html(this.selectedFilesCount)},selectItems:function(b){var a=e(this.options.content).eq(0),d,f=[],g="";this.options.duplicate||this.duplicate();for(var h=this.checkedItems.length,k=0;k<h;k++){b&&(g=b[k].input||[]);var l=this.checkedItems[k];if("undefined"===typeof this.radioButtons||this.radioButtons){d=0;for(var m=this.options.input.length;d<m;d++)"radio"===this.options.input[d].type&&
(this.radioButtons=!0,this.options.input[d].name=this.options.input[d].name.replace(/\[[0-9]*]/,"["+this.selectedFilesCount+"]")),this.radioButtons||(this.radioButtons=!1)}if(d=this.getTemplate(l,g,!!this.silentItems[k]))f.push(d),this.selectedFilesCount++;b||(l=l.split(/:(.+)?/,2),l=l[1]||l[0],this.checkedItems[k]=l)}a.children(".ssi-selectedItemWrapper").append(f);b||this.ssi.$element.trigger("selectAction",[this.checkedItems]);this.checkedItems=[]},getTemplate:function(b,a,d){var c="",e="",h=b.split(/:(.+)?/,
2);d=d||!!a;b=h[1]||b;for(var k=0,l=this.options.input.length;k<l;k++)c+=Ss_input.tools.template(this.template.inputField,{input:(new Ss_input.Input(this.options.input[k],a[k]))[0].outerHTML});a=this.selection.getSelectedInfo(b,"",d);a.displayImage||(e="ssi-empty");return Ss_input.tools.template(this.template[this.options.template].item,{inputName:this.options.inputName,selectionField:a.selection,id:b,className:e,displayImage:a.displayImage,uniqueId:this.ssi.uniqueId,inputs:c,displayName:a.displayName,
dataCollection:h[1]?h[0]:""})},pickData:function(b,a,d){if(!this.options.duplicate&&d&&-1<e.inArray("/:"+b,this.checkedItems))return this.ssi.notify("error",this.translate("existInListError")),this;var c;c=a?e(a):this.ssi.get$mainElementById(b);b=this.ssi.currentCollection.id+":"+(b||a.attr("data-ID"));if(!b)return console.log("Cant't find the id."),this;if(c.hasClass("ssi-checked"))this.checkedFilesCount--,this.removeCheck("",c),this.unCheckItem(b);else{if(0!==this.options.maxItems&&(this.options.duplicate?
this.selectedFilesCount+this.checkedFilesCount:this.checkedFilesCount)===this.options.maxItems){this.ssi.notify("error",Ss_input.tools.replaceText(this.translate("limitError"),this.options.maxItems.toString()));return}this.checkedFilesCount++;this.addCheck("",c);this.checkItem(b,d)}this.ssi.$content.find("#ssi-displayCheckedFiles").html(this.checkedFilesCount);return this},setMChecked:function(b){var a=this.ssi,d,e;b=b||a.$content;var g=this.checkedItems.filter(function(b){e=b.split(/:(.+)?/,2);return e[0]==
a.currentCollection.id||!e[1]});b.find(".ssi-checked").removeClass("ssi-checked");b.find("span.ssi-checkItem").remove();for(var h=0,k=g.length;h<k;h++)e=g[h].split(/:(.+)?/,2),d=e[1]||e[0],this.addCheck(d,"",b)},removeCheck:function(b,a){var c;c=a?e(a):this.ssi.get$mainElementById(b);c.removeClass("ssi-checked");c.find(".ssi-checkItem").remove()},removeSelectedImage:function(b){b=e(b.target).parents(".ssi-pickItem");var c=b.attr("data-ID");if(!this.options.duplicate){var d=b.attr("data-collection");
Ss_input.tools.removeFromArray(this.selected,d?d+":"+c:c)}b.remove();this.radioButtons&&a(this);this.ssi.$element.trigger("removeAction",c)},addCheck:function(b,a,d){(a?e(a):this.ssi.get$mainElementById(b,d)).append('<span class="icon ssi-checkItem"></span>').addClass("ssi-checked")},checkItem:function(b,a){b instanceof Array?this.checkedItems=this.checkedItems.concat(b):(a&&(this.options.duplicate?this.silentItems[this.checkedItems.length]=a:this.silentItems[Ss_input.tools.arrayValuesInArray(this.checkedItems,
this.selected).length]=a),this.checkedItems.push(b));this.ssi.$content.parents(".ssi-mainModal").find(".ssi-insertBtn").removeClass("disabled").prop("disabled",!1)},unCheckItem:function(b){Ss_input.tools.removeFromArray(this.checkedItems,b);0===this.checkedItems.length&&e(".ssi-insertBtn").addClass("disabled").prop("disabled",!0)},duplicate:function(){var b=Ss_input.tools.arrayValuesInArray(this.checkedItems,this.selected),a=Ss_input.tools.arrayValuesInArray(this.selected,this.checkedItems),d,f;this.selected=
this.checkedItems.slice();for(var g=e(this.options.content).eq(0).children(".ssi-selectedItemWrapper"),h=0;h<a.length;h++)d=a[h].split(/:(.+)?/,2),f=d[1]||d[0],g.find('.ssi-pickItem[data-ID="'+f+'"]').each(function(){var b=e(this),a=b.attr("data-collection");a&&a!=d[0]||b.remove()}),this.selectedFilesCount--;this.checkedItems=b},clearChecked:function(){var b=this.ssi.$content;b.find(".ssi-checkItem").remove();b.find(".ssi-checked").removeClass("ssi-checked");b.find("#ssi-displayCheckedFiles").html(0);
this.duplicate||b.find("#ssi-displaySelectedFiles").html(this.selectedFilesCount-this.checkedFilesCount);this.checkedFilesCount=0;this.checkedItems=[];b.parents(".ssi-mainModal").find(".ssi-insertBtn").addClass("disabled").prop("disabled",!0)},clearSelected:function(){e(this.options.content).eq(0).find(".ssi-pickItem").remove();this.ssi.$content.find("#ssi-displaySelectedFiles").html(this.duplicate?0:this.checkedFilesCount);this.selectedFilesCount=0;this.selected=[]}})})(jQuery);
(function(e){Ss_input.selectionMode.singleSelection=Ss_input.Plugin.extend({template:{imgHolder:'<table class="ssi-pickItem ssi-selectedItemWrapper ssi-itemWrapper"><tr><td class="ssi-imgHolder"><div class="ssi-selectionPreview"><div id="ssi-actions">\x3c!--<a href="#" id="ssi-pickedImg" class="ssi-imgBox ssi-imgPreview ssi-hidden"><div class="icon ssi-imgBoxIcon"></div></a>--\x3e<a href="#" class="ssi-removeChoice ssi-hidden"><div class="icon ssi-removeIcon"></div></a></div><div id="ssi-imgChoose" class="ss-input ssi-pick ssi-selectionPreview"></div></div></td></tr><tr><td><div class="ssi-itemName"></div></td></tr><#=input#></table>',
textHolder:'<div class="ssi-pickItem ssi-selectedItemWrapper ssi-itemWrapper"><span class="ssi-textPreview"></span><#= input #></div><a href="#" class="ssi-removeChoice"><div class="icon ssi-removeIcon"></div></a>',input:'<input  id="<#=name#> type="<#=(type||"text")#>" placeholder="<#=placeholder#>" class="ssi-dataUrlName" value="<#=value#>" name="<#=name#>" />',mainInput:'<input id="<#=name#>" type="hidden" class="ssi-mainInput" name="<#=name#>"/>'},defaults:{template:"imgHolder",defaultValue:"",
selectionField:"",inputName:"ssInput"},init:function(){var a=this.ssi;if(a.$element.is("div"))return a.$element.addClass("ssi-"+this.options.template),a.$element.append(Ss_input.tools.template(this.template[this.options.template],{input:'<input id="'+this.options.inputName+'" type="hidden" class="ssi-mainInput" name="'+this.options.inputName+'"/>'})),this.options.defaultValue&&this.pickData(this.options.defaultValue,"",!0),this.setEvents(),this;console.log("The targeted element is not a div.")},setEvents:function(){var a=
this;this.ssi.$element.on("click",".ssi-removeChoice",function(b){b.preventDefault();a.removeSelectedImage();return!1}).on({mouseenter:function(){e(this).find("#ssi-actions").fadeIn(200)},mouseleave:function(){e(this).find("#ssi-actions").fadeOut(200)}},".ssi-selectionPreview");return this},pickData:function(a,b,c){var d=e(b);a=a||d.attr("data-ID");if(!a)return console.log("Cant't find the id."),this;a=this.selection.getSelectedInfo(a,b,c);b=this.ssi.$element;ssi_modal.closeAll();b.find("a").removeClass("ssi-hidden");
b.find(".ssi-itemName").html(a.displayName);b.find(".ssi-itemWrapper").removeClass("ssi-empty");Ss_input.tools.loadImage(b.find("#ssi-imgChoose"),a.displayImage);b.find(".ssi-mainInput").val(a.selection);c||b.trigger("selectAction",a.selection);return this},removeSelectedImage:function(){var a=this.ssi.$element,b=a.find("#"+this.options.inputName);a.find(".ssi-itemWrapper").removeClass("ssi-empty");a.find("#ssi-imgChoose").css("background-image","");a.find("a").addClass("ssi-hidden");a.find(".ssi-itemName").html("");
a.trigger("removeAction",b.val());b.val("");return this}})})(jQuery);(function(e){Ss_input.selectionMode.textEditorSelection=Ss_input.Plugin.extend({defaults:{method:function(){}},init:function(){this.ssi.mode="textEditorSelect";this.ssi.$element.addClass("ss-input");return this},pickData:function(a,b){var c=this.selection.getSelectedInfo(a,b).selection;"function"===typeof this.options.method&&this.options.method(c);ssi_modal.closeAll();return this}})})(jQuery);
