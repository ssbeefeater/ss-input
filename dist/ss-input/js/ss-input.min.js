...
title:this.language.actions,label:'<div class="icon ssi-actionsIcon"></div>',dropDown:!0,selectionRequired:!0},"menuButton",!1));break;case "options":this.$content.find(this.buttonSelectors.menu).append(this.addButton({id:"ssi-TasksBtn",title:this.language.options,label:'<div class="icon ssi-optionsIcon"></div>',dropDown:!0},"menuButton",!1));break;case "openWith":this.$content.find(this.buttonSelectors.contextMenu).append(this.addButton({id:"ssi-openWithBtn",title:this.language.openWith,label:'<span class="icon ssi-openWithIcon"></span>'+
this.language.openWith,selectionRequired:!0,subMenu:!0},"listButton",!1));break;case "editWith":this.$content.find(this.buttonSelectors.contextMenu).append(this.addButton({id:"ssi-editWithBtn",title:"editWith",label:'<span class="icon ssi-editWithIcon"></span>editWith',subMenu:!0,selectionRequired:!0},"listButton",!1))}};Ss_input.prototype.buttonsInit=function(){var b=this.buttonSelectors;this.$content.find(b.menu).append(a[b.menu]);delete a[b.menu];for(var d in a)try{var e=d.split(","),h;if(0!=(h=
Ss_input.tools.keyExists(e,this.initializedButtons)).length)for(var b=0,k=h.length;b<k;b++)this.initializedButtons[h[b]]=!0,this.coreButtonsInit(h[b]);this.$content.find(d).append(a[d])}catch(l){}a={}};Ss_input.prototype.addButton=function(b,d,g,h){b=Ss_input.tools.toArray(b);d=d||"menuButton";for(var k=[],l=0,m=b.length;l<m;l++)if("bottom"==g&&"modalWindow"===this.options.showTo)this.options.modalOptions.buttons.push(b[l]);else{var n=new Ss_input.Button(b[l],d,this.$content);if(g)if(g instanceof
e)g.append(n);else if("itemButton"!=g){for(var p=[],q=0,r=g.length;q<r;q++)p.push(this.buttonSelectors[g[q]]);a.hasOwnProperty(p.toString())?a[p.toString()].push(n):a[p.toString()]=[n];h&&b[l].selectionRequired&&(this.ssi_excludeList[b[l].className]=h)}k.push(n)}return 1==k.length?k[0]:k};Ss_input.prototype.notify=function(a,d){b.notify(a,{overrideOther:!0,content:d})};Ss_input.prototype.checkExcludedButtons=function(a){var b=this.ssi_excludeList,d=this.get$selectedItems(),h=this.$content.find(".ssi-mustSelect").removeClass("ssi-hidden disabled"),
...
(function(e){function b(a){var b=e(a.options.content).eq(0).find(".ssi-radioGroup");a=0;for(var c=b.length;a<c;a++)(function(a){b.eq(a).find("input").attr("name",function(b,d){e(this).attr("name",d.replace(/\[[0-9]*]/,"["+a+"]"))})})(a)}Ss_input.selectionMode.multiSelection=Ss_input.Plugin.extend({template:{inputField:"<td><#=input#></td>",table:{wrapper:'<table class="ssi-selectedItemWrapper  <#=className#> ssi-multiTable"><tr><th></th><# for (var i = 0, inputLength = inputs.length; i<inputLength; i++) { var input=inputs[i]; #><th><#= input.label #></th><#  } #><th></th></tr></table>',
...
defaults:{template:"table",className:"",duplicate:!0,inputName:"files[]",selectionField:"id",maxItems:0,content:"",input:[],defaultValue:""},init:function(){this.checkedItems=[];this.selected=[];this.silentItems=[];this.checkedFilesCount=this.selectedFilesCount=0;var a=this.ssi;"boxes"==this.options.template&&0<this.options.input.length&&(this.options.template="table");a.$element.addClass("ss-input ssi-multiPickMode");(a=this.template[this.options.template].wrapper)&&e(this.options.content).eq(0).html(Ss_input.tools.template(a,
{className:this.options.className,inputs:this.options.input}));if(this.options.defaultValue){a=this.options.defaultValue.length;0!==this.options.maxItems&&a>this.options.maxItems&&(a-=this.options.maxItems);for(var b=0;b<a;b++)this.checkedItems.push(this.options.defaultValue[b].value);this.selectItems(this.options.defaultValue)}this.options.maxItems&&3<(Math.log(this.options.maxItems)*Math.LOG10E+1|0)&&(this.options.maxItems=9999);this.setButtons();this.setEvents();return this},setButtons:function(){var a=
this;this.ssi.addButton([{label:this.translate("insert"),className:"ssi-mBtn ssi-insertBtn disabled",closeAfter:!0,enableAfter:!0,method:function(){a.selectItems();"modalWindow"!=a.ssi.options.showTo&&a.reset()}},{closeAfter:!0,label:this.translate("cancel"),className:"ssi-mBtn ssi-cancel"}],"menuButton",["bottom"])},setEvents:function(){var a=this,b=this.ssi;b.$element.on("showAction",function(){a.options.duplicate||(a.checkItem(a.selected),a.checkedFilesCount=a.selected.length);b.$content.find("#ssi-menuButtons").after(Ss_input.tools.template(a.template.displayFiles,
{selected:a.options.maxItems?'<span id="ssi-displaySelectedFiles">'+a.selectedFilesCount+"</span>/"+a.options.maxItems:'<span id="ssi-displaySelectedFiles">'+a.selectedFilesCount+"</span>",checked:a.checkedFilesCount,selectedTooltip:a.translate("selectedTooltip"),checkedTooltip:a.translate("checkedTooltip"),checkedBtn:a.translate("checkedBtn"),selectedBtn:a.translate("selectedBtn")}));b.$content.on("click","#ssi-clearChecked",function(){a.clearChecked();return!1}).on("click","#ssi-clearSelected",
function(){a.clearSelected();return!1});b.$element.on("echoItemsAction.ssi silentEchoItemsAction.ssi",function(){a.setMChecked()}).on("closeAction.ssi",function(){a.checkedFilesCount=0;a.checkedItems=[]}).on("appendItemAction.ssi",function(b,d){-1!==e.inArray(d.data.path,a.checkedItems)&&a.addCheck("",d.$element.find(".ssi-canSelect"))}).on("infoCloseAction.ssi",function(){a.setMChecked()}).on("infoShowAction.ssi",function(b,d){a.setMChecked(d)}).on("beforeChangeCollectionAction.ssi",function(){a.selectItems();
a.reset()}).on("removeItemAction.ssi",function(a,b){e.inArray(b,"thisS.checkedItems")})});e(this.options.content).eq(0).on("click",".ssi-removeChoice",function(b){try{a.removeSelectedImage(b),a.selectedFilesCount--,a.ssi.$content.find("#ssi-displaySelectedFiles").html(a.selectedFilesCount)}catch(d){}return!1}).on({mouseenter:function(){e(this).find("#ssi-actions").fadeIn(200)},mouseleave:function(){e(this).find("#ssi-actions").fadeOut(200)}},".ssi-selectionPreview")},reset:function(){this.checkedFilesCount=
0;this.checkedItems=[];this.options.duplicate?(this.ssi.$content.find("#ssi-displayCheckedFiles").html("0"),this.ssi.$content.find(".ssi-checked").removeClass("ssi-checked").find(".ssi-checkItem").remove()):(this.checkItem(this.selected),this.checkedFilesCount=this.selected.length);this.ssi.$content.find("#ssi-displaySelectedFiles").html(this.selectedFilesCount)},selectItems:function(a){var b=e(this.options.content).eq(0),c,f=[],g="";this.options.duplicate||this.duplicate();for(var h=this.checkedItems.length,
k=0;k<h;k++){a&&(g=a[k].input||[]);var l=this.checkedItems[k];if("undefined"===typeof this.radioButtons||this.radioButtons){c=0;for(var m=this.options.input.length;c<m;c++)"radio"===this.options.input[c].type&&(this.radioButtons=!0,this.options.input[c].name=this.options.input[c].name.replace(/\[[0-9]*]/,"["+this.selectedFilesCount+"]")),this.radioButtons||(this.radioButtons=!1)}if(c=this.getTemplate(l,g,!!this.silentItems[k]))f.push(c),this.selectedFilesCount++;a||(l=l.split(/:(.+)?/,2),l=l[1]||
l[0],this.checkedItems[k]=l)}b.children(".ssi-selectedItemWrapper").append(f);a||this.ssi.$element.trigger("selectAction",[this.checkedItems]);this.checkedItems=[]},getTemplate:function(a,b,c){var e="",g="",h=a.split(/:(.+)?/,2);c=c||!!b;a=h[1]||a;for(var k=0,l=this.options.input.length;k<l;k++)e+=Ss_input.tools.template(this.template.inputField,{input:(new Ss_input.Input(this.options.input[k],b[k]))[0].outerHTML});b=this.selection.getSelectedInfo(a,"",c);b.displayImage||(g="ssi-empty");return Ss_input.tools.template(this.template[this.options.template].item,
{inputName:this.options.inputName,selectionField:b.selection,id:a,className:g,displayImage:b.displayImage,uniqueId:this.ssi.uniqueId,inputs:e,displayName:b.displayName,dataCollection:h[1]?h[0]:""})},pickData:function(a,b,c){if(!this.options.duplicate&&c&&-1<e.inArray("/:"+a,this.checkedItems))return this.ssi.notify("error",this.translate("existInListError")),this;var f;f=b?e(b):this.ssi.get$mainElementById(a);a=this.ssi.currentCollection.id+":"+(a||b.attr("data-ID"));if(!a)return console.log("Cant't find the id."),
this;if(f.hasClass("ssi-checked"))this.checkedFilesCount--,this.removeCheck("",f),this.unCheckItem(a);else{if(0!==this.options.maxItems&&(this.options.duplicate?this.selectedFilesCount+this.checkedFilesCount:this.checkedFilesCount)===this.options.maxItems){this.ssi.notify("error",this.translate("limitError").replaceText(this.options.maxItems.toString()));return}this.checkedFilesCount++;this.addCheck("",f);this.checkItem(a,c)}this.ssi.$content.find("#ssi-displayCheckedFiles").html(this.checkedFilesCount);
return this},setMChecked:function(a){var b=this.ssi,c,e;a=a||b.$content;var g=this.checkedItems.filter(function(a){e=a.split(/:(.+)?/,2);return e[0]==b.currentCollection.id||!e[1]});a.find(".ssi-checked").removeClass("ssi-checked");a.find("span.ssi-checkItem").remove();for(var h=0,k=g.length;h<k;h++)e=g[h].split(/:(.+)?/,2),c=e[1]||e[0],this.addCheck(c,"",a)},removeCheck:function(a,b){var c;c=b?e(b):this.ssi.get$mainElementById(a);c.removeClass("ssi-checked");c.find(".ssi-checkItem").remove()},removeSelectedImage:function(a){a=
e(a.target).parents(".ssi-pickItem");var d=a.attr("data-ID");if(!this.options.duplicate){var c=a.attr("data-collection");Ss_input.tools.removeFromArray(this.selected,c?c+":"+d:d)}a.remove();this.radioButtons&&b(this);this.ssi.$element.trigger("removeAction",d)},addCheck:function(a,b,c){(b?e(b):this.ssi.get$mainElementById(a,c)).append('<span class="icon ssi-checkItem"></span>').addClass("ssi-checked")},checkItem:function(a,b){a instanceof Array?this.checkedItems=this.checkedItems.concat(a):(b&&(this.options.duplicate?
this.silentItems[this.checkedItems.length]=b:this.silentItems[Ss_input.tools.arrayValuesInArray(this.checkedItems,this.selected).length]=b),this.checkedItems.push(a));this.ssi.$content.parents(".ssi-mainModal").find(".ssi-insertBtn").removeClass("disabled").prop("disabled",!1)},unCheckItem:function(a){Ss_input.tools.removeFromArray(this.checkedItems,a);0===this.checkedItems.length&&e(".ssi-insertBtn").addClass("disabled").prop("disabled",!0)},duplicate:function(){var a=Ss_input.tools.arrayValuesInArray(this.checkedItems,
this.selected),b=Ss_input.tools.arrayValuesInArray(this.selected,this.checkedItems),c,f;this.selected=this.checkedItems.slice();for(var g=e(this.options.content).eq(0).children(".ssi-selectedItemWrapper"),h=0;h<b.length;h++)c=b[h].split(/:(.+)?/,2),f=c[1]||c[0],g.find('.ssi-pickItem[data-ID="'+f+'"]').each(function(){var a=e(this),b=a.attr("data-collection");b&&b!=c[0]||a.remove()}),this.selectedFilesCount--;this.checkedItems=a},clearChecked:function(){var a=this.ssi.$content;a.find(".ssi-checkItem").remove();
a.find(".ssi-checked").removeClass("ssi-checked");a.find("#ssi-displayCheckedFiles").html(0);this.duplicate||a.find("#ssi-displaySelectedFiles").html(this.selectedFilesCount-this.checkedFilesCount);this.checkedFilesCount=0;this.checkedItems=[];a.parents(".ssi-mainModal").find(".ssi-insertBtn").addClass("disabled").prop("disabled",!0)},clearSelected:function(){e(this.options.content).eq(0).find(".ssi-pickItem").remove();this.ssi.$content.find("#ssi-displaySelectedFiles").html(this.duplicate?0:this.checkedFilesCount);
this.selectedFilesCount=0;this.selected=[]}})})(jQuery);
...
input:'<input  id="<#=name#> type="<#=(type||"text")#>" placeholder="<#=placeholder#>" class="ssi-dataUrlName" value="<#=value#>" name="<#=name#>" />',mainInput:'<input id="<#=name#>" type="hidden" class="ssi-mainInput" name="<#=name#>"/>'},defaults:{template:"imgHolder",defaultValue:"",selectionField:"",inputName:"ssInput"},init:function(){var b=this.ssi;if(b.$element.is("div"))return b.$element.addClass("ssi-"+this.options.template),b.$element.append(Ss_input.tools.template(this.template[this.options.template],
{input:'<input id="'+this.options.inputName+'" type="hidden" class="ssi-mainInput" name="'+this.options.inputName+'"/>'})),this.options.defaultValue&&this.pickData(this.options.defaultValue,"",!0),this.setEvents(),this;console.log("The targeted element is not a div.")},setEvents:function(){var b=this;this.ssi.$element.on("click",".ssi-removeChoice",function(a){a.preventDefault();b.removeSelectedImage();return!1}).on({mouseenter:function(){e(this).find("#ssi-actions").fadeIn(200)},mouseleave:function(){e(this).find("#ssi-actions").fadeOut(200)}},
".ssi-selectionPreview");return this},pickData:function(b,a,d){var c=e(a);b=b||c.attr("data-ID");if(!b)return console.log("Cant't find the id."),this;b=this.selection.getSelectedInfo(b,a,d);a=this.ssi.$element;ssi_modal.closeAll();a.find("a").removeClass("ssi-hidden");a.find(".ssi-itemName").html(b.displayName);a.find(".ssi-itemWrapper").removeClass("ssi-empty");Ss_input.tools.loadImage(a.find("#ssi-imgChoose"),b.displayImage);a.find(".ssi-mainInput").val(b.selection);d||a.trigger("selectAction",
b.selection);return this},removeSelectedImage:function(){var b=this.ssi.$element,a=b.find("#"+this.options.inputName);b.find(".ssi-itemWrapper").removeClass("ssi-empty");b.find("#ssi-imgChoose").css("background-image","");b.find("a").addClass("ssi-hidden");b.find(".ssi-itemName").html("");b.trigger("removeAction",a.val());a.val("");return this}})})(jQuery);
...
"data[i]"))?d>k&&k++:("preview"==h?g=b[l][h]:(g=Ss_input.tools.escapeHtml(b[l][h]),console.log(g),5<g.split(" ").length&&(g='<div class="ssi-longText">'+g+"</div>")),c+=Ss_input.tools.template(this.template.item,{fieldData:g}),a++);f+=Ss_input.tools.template(this.template.row,{header:this.translate(h,this.options.translateFields),items:c});if(0==a)return!1}return Ss_input.tools.template(this.template.table,{rows:f})},getFilesInfo:function(){var b=this.ssi,a=b.getSelectedData();if(a.length&&(this.handleData(a),
a=this.getTemplate(a))){var d=e(a);b.createWindow({title:this.translate("details"),sizeClass:"auto",className:"ssi-infoModal",outSideClose:!1,content:d,buttons:{className:"ssi-mBtn",label:this.translate("ok"),closeAfter:!0},onClose:function(){b.$element.trigger("infoCloseAction.ssi")},onShow:function(){b.$element.trigger("infoShowAction.ssi",[d])}})}}})})(jQuery);
...
this.options.readOnly=this.options.readOnly||!0},openFile:function(b,a){var d=this;this.sendRequest(b,"",function(c){c=e('<textarea id="ssi-textArea" class="ssi-textArea" '+(d.options.readOnly?"readonly":"")+">"+c+"</textarea>");d.createWindow(c,Ss_input.tools.basename(b));"function"==typeof a&&a(c)},"GET")},saveFile:function(b,a){var d=this;this.sendRequest(b,a,function(a){d.ssi.notify("success",d.translate("fileSaved"))},"UPDATE");return this},createWindow:function(b,a){this.modalOptions.content=
b;this.modalOptions.title=a;this.uniqueId="ssi-normalModal"+this.ssi.createWindow(this.modalOptions).numberId;return this},sendRequest:function(b,a,d,c){var f=this.ssi;b=e.extend({},this.options.data,{filePath:b,fileContent:a});c=e.extend({},{data:b,type:c,url:f.fileSystem.options.scriptsPath+"/editFileAction.php"},this.options.ajaxOptions);f.ajaxCall(c,d);return this},getContent:function(){return e("#"+this.uniqueId).find("#ssi-textArea").val()},getPath:function(){return this.path}})})(jQuery);
...
(function(e){Ss_input.fileSystem.download=Ss_input.Plugin.extend({init:function(){this.setButtons()},setButtons:function(){var b=this;this.ssi.addButton({label:'<span class="icon ssi-downloadIcon"></span>'+this.translate("download"),method:function(){b.download()},className:"ssi-openInBtn",selectionRequired:!0},"listButton",["contextMenu"],'condition:(field:(mimeType)!="directory")')},download:function(){for(var b=this.ssi,a=b.getSelectedField("name").length,d=0;d<a;d++){var c=b.getSelectedField("name")[d],
e=document.createElement("a");e.download=c;e.href=b.fileSystem.getPath(c);document.body.appendChild(e);e.click();document.body.removeChild(e);delete e}}})})(jQuery);
