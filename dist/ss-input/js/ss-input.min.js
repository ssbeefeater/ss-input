...
k=0;k<h;k++){a&&(g=a[k].input||[]);c=this.checkedItems[k];if("undefined"===typeof this.radioButtons||this.radioButtons)for(var l=0,m=this.options.input.length;l<m;l++)"radio"===this.options.input[l].type&&(this.radioButtons=!0,this.options.input[l].name=this.options.input[l].name.replace(/\[[0-9]*]/,"["+this.selectedFilesCount+"]")),this.radioButtons||(this.radioButtons=!1);if(c=this.getTemplate(c,g,!!this.silentItems[k]))f.push(c),this.selectedFilesCount++}b.children(".ssi-selectedItemWrapper").append(f);
this.checkedItems=[]},getTemplate:function(a,b,c){var e="",g="",h=a.split(/:(.+)?/,2);c=c||!!b;a=h[1]||a;for(var k=0,l=this.options.input.length;k<l;k++)e+=Ss_input.tools.template(this.template.inputField,{input:(new Ss_input.Input(this.options.input[k],b[k]))[0].outerHTML});b=this.selection.getSelectedInfo(a,"",c);b.displayImage||(g="ssi-empty");return Ss_input.tools.template(this.template[this.options.template].item,{inputName:this.options.inputName,selectionField:b.selection,id:a,className:g,displayImage:b.displayImage,
uniqueId:this.ssi.uniqueId,inputs:e,displayName:b.displayName,dataCollection:h[1]?h[0]:""})},pickData:function(a,b,c){if(!this.options.duplicate&&c&&-1<e.inArray("/:"+a,this.checkedItems))return this.ssi.notify("error",this.translate("existInListError")),this;var f;f=b?e(b):this.ssi.get$mainElementById(a);a=this.ssi.currentCollection.id+":"+(a||b.attr("data-ID"));if(!a)return console.log("Cant't find the id."),this;if(f.hasClass("ssi-checked"))this.checkedFilesCount--,this.removeCheck("",f),this.unCheckItem(a);
else{if(0!==this.options.maxItems&&(this.options.duplicate?this.selectedFilesCount+this.checkedFilesCount:this.checkedFilesCount)===this.options.maxItems){this.ssi.notify("error",this.translate("limitError").replaceText(this.options.maxItems.toString()));return}this.checkedFilesCount++;this.addCheck("",f);this.checkItem(a,c)}this.ssi.$content.find("#ssi-displayCheckedFiles").html(this.checkedFilesCount);return this},setMChecked:function(a){var b=this.ssi,c,e;a=a||b.$content;var g=this.checkedItems.filter(function(a){e=
a.split(/:(.+)?/,2);return e[0]==b.currentCollection.id||!e[1]});a.find("span.ssi-checkItem").remove();for(var h=0,k=g.length;h<k;h++)e=g[h].split(/:(.+)?/,2),c=e[1]||e[0],this.addCheck(c,"",a)},removeCheck:function(a,b){var c;c=b?e(b):this.ssi.get$mainElementById(a);c.removeClass("ssi-checked");c.find(".ssi-checkItem").remove()},removeSelectedImage:function(a){a=e(a.target).parents(".ssi-pickItem");if(!this.options.duplicate){var d=a.attr("data-ID"),c=a.attr("data-collection");Ss_input.tools.removeFromArray(this.selected,
c?c+":"+d:d)}a.remove();this.radioButtons&&b(this)},addCheck:function(a,b,c){(b?e(b):this.ssi.get$mainElementById(a,c)).append('<span class="icon ssi-checkItem"></span>').addClass("ssi-checked")},checkItem:function(a,b){a instanceof Array?this.checkedItems=this.checkedItems.concat(a):(b&&(this.options.duplicate?this.silentItems[this.checkedItems.length]=b:this.silentItems[Ss_input.tools.arrayValuesInArray(this.checkedItems,this.selected).length]=b),this.checkedItems.push(a));this.ssi.$content.parents(".ssi-mainModal").find(".ssi-insertBtn").removeClass("disabled").prop("disabled",
!1)},unCheckItem:function(a){a=a.split(/:(.+)?/,2);a=a[1]||a[0];Ss_input.tools.removeFromArray(this.checkedItems,a);0===this.checkedItems.length&&e(".ssi-insertBtn").addClass("disabled").prop("disabled",!0)},duplicate:function(){var a=Ss_input.tools.arrayValuesInArray(this.checkedItems,this.selected),b=Ss_input.tools.arrayValuesInArray(this.selected,this.checkedItems),c,f;this.selected=this.checkedItems.slice();for(var g=e(this.options.content).eq(0).children(".ssi-selectedItemWrapper"),h=0;h<b.length;h++)c=
b[h].split(/:(.+)?/,2),f=c[1]||c[0],g.find('.ssi-pickItem[data-ID="'+f+'"]').each(function(){var a=e(this),b=a.attr("data-collection");b&&b!=c[0]||a.remove()}),this.selectedFilesCount--;this.checkedItems=a},clearChecked:function(){var a=this.ssi.$content;a.find(".ssi-checkItem").remove();a.find(".ssi-checked").removeClass("ssi-checked");a.find("#ssi-displayCheckedFiles").html(0);this.duplicate||a.find("#ssi-displaySelectedFiles").html(this.selectedFilesCount-this.checkedFilesCount);this.checkedFilesCount=
0;this.checkedItems=[];a.parents(".ssi-mainModal").find(".ssi-insertBtn").addClass("disabled").prop("disabled",!0)},clearSelected:function(){e(this.options.content).eq(0).find(".ssi-pickItem").remove();this.ssi.$content.find("#ssi-displaySelectedFiles").html(this.duplicate?0:this.checkedFilesCount);this.selectedFilesCount=0;this.selected=[]}})})(jQuery);
